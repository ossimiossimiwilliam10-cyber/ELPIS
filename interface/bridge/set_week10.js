...
const ensureTDs = (matName, count) => {
  const mat = findMatiere(matName);
  if (!mat) return;
  const existing = db.prepare(`SELECT count(*) as c FROM exercices WHERE type='TD' AND matiere_id = ?`).get(mat.id).c;
  if (existing < count) {
    const insert = db.prepare(`INSERT INTO exercices (id, type, titre, nombrePratiques, tempsMoyen, difficulte, difficulteInitiale, matiere_id) VALUES (?, 'TD', ?, 0, 0, 'Normale', 'Normale', ?)`);
    for (let i = existing + 1; i <= count; i++) {
      insert.run(crypto.randomUUID(), `TD ${i}`, mat.id);
    }
    console.log(`Created ${count - existing} TDs for ${matName}`);
  }
};
...
db.transaction(() => {
  // 1. Update Exam Dates
  updateExamDate("STS", "2026-11-09");
  updateExamDate("AMLS", "2026-11-09");

  // 2. Ensure structures exist
  ensureCMs("Algèbre", 5);
  ensureCMs("Analyse", 5);
  ensureTDs("Programmation", 5);

  // 3. Update Dates for CMs
  const datesCM = [
    { nom: "Algèbre", cm: 'CM 4', date: '2026-11-12' },
    { nom: "Analyse", cm: 'CM 4', date: '2026-11-12' }
  ];
  ...
  const datesTD = [
    { nom: 'Langues', td: 'TD 7', date: '2026-11-10' },
    { nom: 'Programmation', td: 'TD 3', date: '2026-11-10' },
    { nom: 'Introduction aux Systèmes Electroniques', td: 'TD 3', date: '2026-11-10' },
    { nom: 'Mécanique du solide', td: 'TD 3', date: '2026-11-12' }
  ];
  ...
  const datesTP = [
    { nom: "Architecture des systèmes d'exploitation", tp: 'TP 2', date: '2026-11-09' }
  ];
...
console.log('Finished updating week 10 schedule.');
