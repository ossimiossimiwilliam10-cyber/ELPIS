...
const C_EXECUTABLE = path.join(ROOT_DIR, 'build', `moteur_config${IS_WINDOWS ? '.exe' : ''}`);

const COURS_FILE = path.join(ROOT_DIR, 'espoir_cours.json');
const INCOMING_COURS_FILE = path.join(__dirname, 'incoming_cours.json');
const COURS_EXECUTABLE = path.join(ROOT_DIR, 'build', `moteur_cours${IS_WINDOWS ? '.exe' : ''}`);

const HISTORIQUE_FILE = path.join(ROOT_DIR, 'espoir_historique.json');

const ORCHESTRATEUR_EXECUTABLE = path.join(ROOT_DIR, 'build', `moteur_principal${IS_WINDOWS ? '.exe' : ''}`);
