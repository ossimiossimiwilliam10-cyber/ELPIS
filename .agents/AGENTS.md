...
- **Séparation de l'Évaluation (Anki)** : Lors de la conception ou de la modification de l'interface utilisateur pour les sessions d'entraînement (ex: ExerciceCard), les tâches de type ANKI ne doivent **jamais** exposer les contrôles de difficulté propres à ELPIS (les boutons de difficulté ou les boutons FSRS : À revoir, Difficile, Bien, Parfait). 
- Anki gère sa propre répétition espacée. L'interface d'ELPIS pour une tâche Anki doit se limiter à un bouton unique de validation (ex: "Fait") et un chronomètre/input de temps pour tracker la durée de la session.
</RULE[anki_task_isolation]>


<RULE[offline_first_id_generation]>
- **Tout élément synchronisé naît avec son identifiant.** Toute création côté
  interface d'un objet qui entre dans un document synchronisé — licence,
  semestre, UE, matière, CM, TD, TP, annale, langue, lien, projet, phase,
  vidéo — doit poser son `id` au moment même de sa création, avec
  `crypto.randomUUID()` et un repli si l'API manque. Ne jamais compter sur la
  base pour l'attribuer à l'écriture : entre la création et l'écriture, il y a
  la synchronisation.
- **Pourquoi** : `interface/web/src/utils/fusion.js` apparie les listes par
  identifiant (`FORMES` déclare `{ type: 'liste', cle: 'id' }`). Un élément sans
  clé ne peut pas être apparié. Constaté le 4 septembre 2026 : dix chapitres
  saisis sur le PC revenaient à six, sans message ni erreur — et comme le
  résultat de la fusion est réécrit des deux côtés, la saisie était perdue
  partout.
- **Filet, pas dispense** : la fusion conserve désormais les éléments sans clé
  au lieu de les écarter en silence, et le signale dans son compte rendu. Ce
  filet couvre les données écrites par une version antérieure ; il ne dispense
  pas de la règle, car un élément sans identifiant reste inappariable d'un
  appareil à l'autre.
</RULE[offline_first_id_generation]>
