...
- **D�l�gation aux Linters Standards** : Lors de la conception de syst�mes d'auto-correction ou d'audit de code, ne jamais utiliser de scripts regex personnalis�s. D�l�guer l'analyse et la correction s�re aux standards de l'industrie (ESLint pour JS, Ruff/Black pour Python). L'agent personnalis� doit servir d'orchestrateur (lancement, parsing de rapport JSON, rollbacks) et non de moteur d'analyse de code.
</RULE[agent_architecture_linters]>


<RULE[project_ci_cd_sync]>
- **Compilation et Push Systématique (Sync Local/Remote)** : À la fin de chaque tâche de modification de code, je dois **TOUJOURS** :
  1. Compiler le code pour m'assurer qu'il ne casse rien (ex: 
pm run build dans le répertoire approprié comme interface/web).
  2. Ajouter, commiter et pusher les modifications sur Git (git add ., git commit, git push) pour garder le dépôt distant synchronisé avec le local.
  3. Utiliser le format **Conventional Commits** pour les messages (ex: eat: ..., ix: ..., docs: ...).
</RULE[project_ci_cd_sync]>

<RULE[project_systematic_docs]>
- **Documentation Systématique** : Pour chaque section modifiée, ajoutée ou supprimée, je dois **SYSTÉMATIQUEMENT** mettre à jour la documentation du projet.
